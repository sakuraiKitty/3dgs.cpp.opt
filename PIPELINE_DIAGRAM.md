...
│   单帧 dispatch = 4×96 = 384（idle，⑥跳过）/ 5×96 = 480（拖拽 +⑦）            │
└───────────────────────────┬──────────────────────────────────────────────────┘
                            ▼
┌──────────────────────────────────────────────────────────────────────────────┐
...
- **MPM 粒子数 13,356** → `(13356+63)/64 = 211` workgroups/阶段（local_size=64，占用率优化）；96 substeps × 4 阶段（⑥HomeSpring 跳过，PinFrozen/GridFreeze 已融合）= **384 次 compute dispatch/帧**（idle）；拖拽时 +⑦DragBC = 5×96 = **480 次/帧**。
...
*基于 phys-sim 分支 2026-07-02 状态（home-spring 关闭 / 纯 FCR / carnation substeps=96 grid=48 CFL 0.635 / PinFrozen+GridFreeze 融合 / local_size=64 / ~30 FPS，P2G tiling 失败）。dispatch size 随场景/窗口尺寸变化。*
